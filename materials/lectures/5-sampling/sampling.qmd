---
title: "Methods Relying on Sampling: case for parallel computing"
format: 
  revealjs:
    slide-number: c/t
    width: 1200
...
## K-fold Crossvalidation {background-image="/pics/fork/fork8.png" background-size="80%" background-position="bottom" .smaller}

::: {.incremental}
- Let $y = f(x, \beta, \alpha)$ be a model and $\widehat{y} = f(x, \widehat{\beta}, \alpha)$ be the model prediction of $y$
  - $(y, x)$ are data, $\beta$ are model parameters, and $\alpha$ are model hyperparameters
    - Example: Random forest $\beta$ are split-points, $\alpha = n_\min$ is minimum number of nodes in a tree
...
::: aside
Adapted from: Parallel Statistical Computing with R: An Illustration on Two Architectures [arXiv:1709.01195](https://arxiv.org/abs/1709.01195)
:::

## `rf_serial.r`  
```{.r code-line-numbers="|5-9|11|12|14"}
{{< include _code/rf_serial.R >}}
```

## `rf_mc.r`
```{.r code-line-numbers="|1|4|6-10|12|13|14"}
{{< include _code/rf_mc.R >}} 
```
