...
## Yellow Trip Data `/projects/bckj/TLC_yellow`
...
**arrow** (parquet 0.423GB)
```{.cell-output-stdout}
> system.time({tlc2009.02 = arrow::read_parquet("/projects/bckj/TLC_yellow/year=2009/month=2/yellow_tripdata_2009-02.parquet")})
   user  system elapsed 
  3.363   1.981  12.236 
```
**readr** (csv 1.7GB)
...
```{.cell-output-stdout}
> system.time({tlc2009.02csv2 = data.table::fread("/projects/bckj/TLC_yellow_csv/year=2009/month=2/part-0.csv")})
|--------------------------------------------------|
|==================================================|
   user  system elapsed 
...
> tlc2009.02 = tlc %>% filter(year == 2009, month == 2) %>% collect()
```
