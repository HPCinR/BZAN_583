...
**Each group's project plan must be defined and approved by the end of the second week (Friday, March 29).** The project will consist of a **specific large data set** of your choosing and a **hypothesis that will be examined** with statistical and machine learning methods. The main objective is to successfully exercise several HPC methods (e.g. faster analysis or larger data enabled by methods learned in class). A great discovery from the data is not required, although that would be the icing on the cake. 

#### Grading:

The weekly assignments are worth 12% each and the project with presentation is 28%. While each group is assigned a grade that all members receive, at the end of the course all members will evaluate and specify the kind of their own and others' relative contributions. These evaluations will be considered in adjusting the grades.
